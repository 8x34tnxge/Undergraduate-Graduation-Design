    ...
    def run(
        ...
        def updateLocalSchedule(
            ...
            if (not maximize and value > self.localValue) or (
                ...
                if maximize:
                    deltaValue *= -1
                compulsoryProb = np.exp(deltaValue / temperate)
                prob = random.random()
        ...
        while temperate > terminatedTemperate:
            ...
            tbar.set_description_str(f"Best Value: {self.bestValue} Current Temperate: {temperate:.6f}")
            for epoch in tbar:
...
class AdaptiveSimulatedAnnealing(SimulatedAnnealing):
    ...
    def run(
        ...
        for epoch in tbar:
            ...
            tbar.set_description_str(f"Local Value: {self.localValue} Best Value: {self.bestValue} Current Temperate: {temperate:.6f}")
